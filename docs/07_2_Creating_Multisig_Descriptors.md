...
```
msdesc="wsh(sortedmulti(2,$pubkey3,$pubkey4))"
```

Bitcoin Core always requires a checksum for its descriptors, which you can retrieve with `getdescriptorinfo`.

```
mscs=$(bitcoin-cli getdescriptorinfo $msdesc | jq -r '.checksum')
```

You can then create a descriptor with the checksum:

```
msdescwithcs=$msdesc#$mscs
echo $msdescwithcs
...
You've skipped right past the nead to use `createmultisig` to create
your address. Instead, you're now ready to import the descriptor.
...
You don't have to depend on the Bitcoin Core commands! As you grow
increasingly confident with Bitcoin, there will be many things you can
do on your own. Descriptors are one of the best tools for this,
because they let you create a variety of addresses just by knowing how
they're constructed. Even when you're creating complex multisig
addresses, you can fall back on descriptors ... and the results may be
even better than using `bitcoin-cli`, because you can choose to sort
the addresses, which is not the default for Bitcoin Core.
